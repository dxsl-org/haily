...
async fn resumed_run_relaunches_and_is_killable_via_its_token_under_the_same_run_id() {
    ...
    // The relaunch reuses the SAME run_id for its FIRST internal pipeline_runs row (the shared
    // launch helper's registry key) — killable immediately, proving re-registration, not just
    // the checkpoint fallback.
    let killed = tokio::time::timeout(std::time::Duration::from_secs(2), async {
        loop {
            if handle.run_control_registry().cancel(&run.id) {
                break true;
            }
    ...
    .await
    .expect("the resumed run must register a live token under its own run_id");
    assert!(killed);
}
