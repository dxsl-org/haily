...
fn init_git_repo() -> TempDir {
    ...
    let init = Command::new("git").args(["init"]).current_dir(p).output().expect("git init");
...
async fn cleanup_removes_directory() {
    ...
    assert!(!wt_path.exists(), "cleanup must remove the worktree directory");
}
...
async fn diff_empty_on_clean_worktree() {
    ...
    assert!(diff.is_empty(), "clean worktree must produce no diff, got:\n{diff}");
...
async fn diff_captures_untracked_file() {
    ...
    let diff = wt.diff().await.unwrap();
    assert!(diff.contains("hello.txt"), "diff must mention the new file:\n{diff}");
    assert!(diff.contains("+world"), "diff must show the file content:\n{diff}");
...
async fn with_ephemeral_worktree_returns_value_and_diff() {
    ...
    let (value, diff) = EphemeralWorktree::with_ephemeral_worktree(
        repo.path(),
        |wt_path| async move {
    ...
    assert_eq!(value, 99);
    assert!(diff.contains("output.txt"), "diff must capture file written in closure:\n{diff}");
}
...
async fn with_ephemeral_worktree_cleans_up_on_completion() {
    ...
    let (_, _) = EphemeralWorktree::with_ephemeral_worktree(
        repo.path(),
        |wt_path| {
    ...
    if let Some(p) = captured_path {
        assert!(!p.exists(), "worktree must be removed after with_ephemeral_worktree completes");
    }
}
