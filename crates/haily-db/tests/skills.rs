...
async fn decay_called_twice_within_window_second_call_is_noop() {
    ...
    let first_run = skills::apply_exponential_decay(&db, LAMBDA, ARCHIVE_BELOW)
        .await
        .unwrap();
    assert_eq!(
        first_run, 1,
    ...
    let second_run = skills::apply_exponential_decay(&db, LAMBDA, ARCHIVE_BELOW)
        .await
        .unwrap();
    assert_eq!(
        second_run, 0,
...
async fn decay_runs_again_after_guard_window_elapses() {
    ...
    skills::apply_exponential_decay(&db, LAMBDA, ARCHIVE_BELOW)
        .await
        .unwrap();
    ...
    let third_run = skills::apply_exponential_decay(&db, LAMBDA, ARCHIVE_BELOW)
        .await
        .unwrap();
    assert_eq!(
        third_run, 1,
