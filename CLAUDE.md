...
| `crates/haily-app/src/slash_registry/` | Data-driven slash command registry (Unified Chat UI P02): `AuthoredRegistry` + gate-filtered `active_skills()`; built-in verbs + authored/synthesized skill commands; force-inject re-validates vs SkillGates at injection site |
| `crates/haily-app/src/run_control/` | Run control service (Unified Chat UI P06): `kill_run`, `pause_run`, `resume_run` commands; run_id pre-generated at launch via shared helper; registry cleanup on terminal-or-paused transition; local-GUI-only (not bridged to mobile) |
| `crates/haily-app/src/notify.rs` | OS notification sender (Unified Chat UI P07): focus-gated (fires only when window unfocused/minimized); detached-spawn + timeout; respects proactive coalescing |
| `crates/haily-core/src/permission_mode.rs` | Named approval ladder (Unified Chat UI P11): `approval.mode` ∈ {manual (prompt all) | accept_edits (auto reversible) | auto (auto all incl. irreversible)}; DB-persist before flipping live handle; auto-approved irreversible writes get record-only journal row |
| `crates/haily-kms/src/skill_editor/` | Skill editor service (Unified Chat UI P08): atomic 4-field→markdown + manifest write (rename manifest last); `skill_versions` table + revert + boot fallback on corrupt manifest; tamper recovery surfaced |
| `crates/haily-db/src/queries/run_events.rs` | Pipeline run event persistence (Unified Chat UI P05): one writer per run (`spawn_run_event_bridge`), AUTOINCREMENT id for replay order, all variants except StageOutput (text NOT persisted — credential-leak fix), reconcile against `pipeline_runs` on startup |
| `src/lib/components/` | Chat + management UI (Unified Chat UI): IconRail, ChatStream, ChatInput, ChatBubble, RunProgressCard, SlashPalette, ApprovalQueue; RunsList, RunRow, RunDrillIn, RunTelemetry; WorkspacesScreen, WorkspacesPanel; SkillsScreen, SkillEditor, SkillVersions |
